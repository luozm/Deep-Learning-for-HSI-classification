...
Builds the CNN model for HSI data.

Implements the _inference/loss/training pattern_ for model building.
1. cnn_2d() - Builds the model as far as is required for running the network forward to make predictions.
2. loss() - Adds to the inference model the layers required to generate loss.
3. training() - Adds to the loss model the Ops required to generate and apply gradients.
4. evaluation() - Evaluate the quality of the logits at predicting the label.
...
def cnn_test(images, is_training):
    ...
    l_conv1 = conv2d(images, 10, [7, 7],
                            strides=[1, 1], activation=relu, name='Conv1')
